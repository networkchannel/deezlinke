...
import { ArrowRight, Sparkles, Zap, Shield, Clock, Users, User, ChevronDown, Play } from "lucide-react";
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        {(deezer.artists?.length > 0 || deezer.tracks?.length > 0) && (
          <Reveal>
            <section className="mb-16 sm:mb-20">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-bold text-t-primary mb-1">
              ...
              {deezer.artists?.length > 0 && (
                <div className="relative overflow-hidden mb-8">
                  <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide snap-x snap-mandatory">
                    {deezer.artists.slice(0, 12).map((artist) => (
                      <motion.div
                        key={artist.id}
                        ...
                        <GlassCard className="p-4 w-40">
                          ...
                          <p className="text-sm font-semibold text-t-primary truncate">{artist.name}</p>
                          <p className="text-xs text-t-muted">
                            {(artist.nb_fan / 1000000).toFixed(1)}M {lang === "fr" ? "fans" : "fans"}
                          </p>
                        </GlassCard>
                      </motion.div>
                    ))}
              ...
              {deezer.tracks?.length > 0 && (
                <GlassCard className="p-6">
                  <h3 className="text-lg font-bold text-t-primary mb-4">
                    {lang === "fr" ? "Top Titres" : "Top Tracks"}
                  </h3>
                  <div className="space-y-3">
                    {deezer.tracks.slice(0, 5).map((track, i) => (
                      <div key={track.id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-surface-hover transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-t-primary truncate">{track.title}</p>
                          <p className="text-xs text-t-muted truncate">{track.artist_name}</p>
                        </div>
                        {track.preview && (
                          <button className="shrink-0 w-8 h-8 rounded-full glass flex items-center justify-center hover:bg-accent hover:text-white transition-colors">
